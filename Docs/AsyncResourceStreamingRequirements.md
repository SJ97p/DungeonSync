...
| 키 | 동작 |
|---|---|
| `F7` | 동일한 리소스 세트를 메인 스레드에서 동기 로딩 |
| `F10` | 동일한 리소스 세트를 작업 스레드에서 비동기 디코딩 |
| `F11` | 엔진 진단 오버레이 표시/숨김 |

F8/F9 자동 렌더링 벤치마크와 충돌하지 않도록 해당 세션 실행 중에는 F7/F10 요청을 거부한다.
...
- F7 동기 테스트에서 메인 스레드 히치가 Frame Max에 기록됨
- F10 비동기 테스트에서 렌더 루프가 유지됨
...
4. F7/F10 비교 시나리오와 스트리밍 통계를 구현한다.
