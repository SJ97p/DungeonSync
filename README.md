...
> **구분:** 개인 연구개발 · **인원:** 1인 개발 · **기간:** 2026.08.10 – 2026.08.22

Unity 클라이언트를 개발하며 체감했던 프레임 히치, Draw Call, 리소스 로딩 문제는 엔진의 표현 방식은 달라도 결국 CPU가 GPU에 무엇을 언제 제출하고, 프레임 안에서 어떤 작업이 오래 붙잡히는가의 문제로 이어진다고 생각했습니다. DungeonSync는 그 경계를 C++20과 DirectX 11 환경에서 직접 관찰하고, 같은 조건에서 바꾼 결과를 수치로 확인하기 위해 만든 엔진 연구 포트폴리오입니다.

이 프로젝트는 완성된 상용 엔진이나 기존 라이브 코드의 개선 사례가 아닙니다. 대신 작은 2.5D 던전 실행 환경에서, “플레이어가 불쾌하게 느끼는 멈춤과 지연을 어떻게 관찰하고 줄일 것인가”라는 질문을 재현 가능한 실험으로 다룹니다.
...
인스턴싱을 단순히 “Draw Call이 적다”는 설명으로 끝내지 않기 위해, 동일한 `RenderItem` 배치를 두 방식으로 제출하고 CPU·GPU·Present 구간을 나눠 비교했습니다.
...
총 로딩 시간은 유사하지만 메인 스레드의 단일 프레임 정지를 줄였습니다. 즉, 작업량을 숨긴 것이 아니라 **플레이 루프와 작업을 분리하고 프레임 예산에 맞게 분산**한 결과입니다. 이 비교값은 동일한 텍스처 7장과 현재 장비를 기준으로 한 실험 결과입니다.
...
Broad phase는 정확한 피격 판정이 아니라 **놓치지 않는 저비용 후보 수집 단계**이며, narrow phase가 실제 공격 shape를 판정합니다. 이 데모는 2.5D 평면에서 비교적 균일하게 배치된 몬스터와 고정 반경 공격이 중심이어서 Uniform Spatial Grid를 선택했습니다. 객체 밀도가 크게 달라지거나 월드가 커지면 Quadtree/BVH 등 다른 구조를 비교해야 합니다.
...
> Unity에서 경험한 플레이 감각의 문제를 C++/DX11의 CPU·GPU·리소스 경계까지 내려가 다시 관찰하고, 측정 구간을 분리한 뒤 개선 효과와 한계를 수치로 설명할 수 있습니다.
